...
--Ex1: 
CREATE TABLE interview.dbo.PersonDetails
(
 SNO INT	 
,FullName VARCHAR(50)	 
...
--Creating a table by checking the existance of the table in the memory

SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'PersonDetails'
SELECT * FROM sys.sysobjects WHERE type = 'U' AND name = 'PersonDetails'
SELECT * FROM sys.objects WHERE type_desc = 'USER_TABLE' AND name = 'PersonDetails'


-----------


IF EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'PersonDetails')
BEGIN 
	PRINT 'Table PersonDetails is already exists'
END
ELSE 
BEGIN 
	PRINT 'Table PersonDetails does not exists'
END
...
IF NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'PersonDetails')
BEGIN
...
(
	...
	WHERE TABLE_NAME = 'PersonDetails' 
	AND COLUMN_NAME = 'Qualification2'
)
BEGIN
	ALTER TABLE PersonDetails
	ADD Qualification2 VARCHAR(100)
END
...
:
:
--Ex:  Delete all the persons where passed out year is less than 2009 from PersonDetails

SELECT * FROM PersonDetails
WHERE PassedOutYear < 2009
...
--Ex2:  Delete all the persons where passed out year is 2010 or 2013 from PersonDetails
--A1
	SELECT * FROM PersonDetails
	WHERE SNO IN (SELECT SNO FROM PersonDetails WHERE PassedOutYear IN(2010,2013))
...
--A2
	SELECT * FROM PersonDetails
	WHERE SNO IN (SELECT SNO FROM PersonDetails WHERE PassedOutYear = 2010 OR PassedOutYear = 2013)
...
SELECT 
	...
	,EmailID
	,LocationCity
FROM PersonDetails_Copy
WHERE SNO IN (SELECT SNO FROM PersonDetails_Copy WHERE PassedOutYear IN(2010,2013))

--Ex3:  Delete all the persons where passed out year is >=2011 AND <=2014 from PersonDetails
--A1
	SELECT * FROM PersonDetails
	WHERE SNO IN (SELECT SNO FROM PersonDetails WHERE PassedOutYear BETWEEN 2011 AND 2014)
...
--A1
	SELECT * FROM PersonDetails
	WHERE SNO IN (SELECT SNO FROM PersonDetails WHERE PassedOutYear >= 2011 AND PassedOutYear <= 2014)
