...
STUFF,REPLACE
	SELECT STUFF('Banagalore',1,7,'Mys') as col1,REPLACE('Banagalore','Banagal','Mys') as clo2
REPLICATE
	DECLARE @ID INT = 10-- @String VARCHAR(10)
	SELECT REPLICATE('0',4)+ CAST(@ID AS VARCHAR(10))
	SELECT REPLICATE('1',4)+ @ID 
...
select *  into  PersonDetails2 FROM EmpDetails_WithConstraints
